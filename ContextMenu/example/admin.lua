...
AddEventHandler('adminMenu:allow', function()
    ...
    ECM:Register(function(screenPosition, hitSomething, worldPosition, hitEntity, normalDirection)
        ...
        -- Création du sous-menu Debug dans Admin
        local debugMenu = ECM:AddSubmenu(adminMenu, "Debug")

        -- Option pour afficher les informations sur une entité
        local entityInfoItem = ECM:AddItem(debugMenu, "Afficher les infos de l'entité")
        ECM:OnActivate(entityInfoItem, function()
            if DoesEntityExist(hitEntity) then
                ...
                local entityHeading = GetEntityHeading(hitEntity)

                print(string.format("^2Type: %s, Coords: [X: %.2f, Y: %.2f, Z: %.2f], Heading: %.2f^0", entityType, entityCoords.x, entityCoords.y, entityCoords.z, entityHeading))
                TriggerEvent('chat:addMessage', {
                    args = {
                        "DEBUG", string.format("Type: %s, Coords: [X: %.2f, Y: %.2f, Z: %.2f], Heading: %.2f", entityType, entityCoords.x, entityCoords.y, entityCoords.z, entityHeading)
                    }
                })
            else
                print("^1Aucune entité détectée.^0")
                TriggerEvent('chat:addMessage', {
                    args = { "DEBUG", "Aucune entité détectée." }
                })
            end
        end)
...
-- Fonction Nom véhicule
-- local function GetEntityModelName(entity)
--     if DoesEntityExist(entity) then
--         local modelHash = GetEntityModel(entity)
--         return GetDisplayNameFromVehicleModel(modelHash) or "Modèle inconnu"
--     end
